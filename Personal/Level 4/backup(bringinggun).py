...
def solution(dimensions, your_position, trainer_position, distance):
    ...
    def valid(v):
        ...
        while True:
            ...
            elif v[0] == 0 and (trainer_position[0] - pos[0]) == 0:
                x_time = 0
            else:
                print('[x] wrong vector')

            if v[1] != 0:
                y_time = (trainer_position[1] - pos[1]) / v[1]
            elif v[1] == 0 and (trainer_position[1] - pos[1]) == 0:
                y_time = 0
            else:
                print('[y] wrong vector')'''
            ...
            if (x_time == y_time or (x_satisfied and y_time > 0) or (y_satisfied and x_time > 0)):
                dist += ((trainer_position[0] - pos[0])**2 + (trainer_position[1] - pos[1])**2)**(0.5)
                if dist <= distance:
                    if hitting_ourselvs:
                        if x_time < x_time_ourselves:
                            return True
                        else:
                            return False
                    return True
            ...
            # Check if we have exceeded distance
            dist += ((newPos[0] - pos[0])**2 + (newPos[1] - pos[1])**2)**(0.5)
            if dist > distance:
                return False
